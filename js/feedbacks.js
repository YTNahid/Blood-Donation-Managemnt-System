...
const renderFeedbacks = (feedbacks, role) => {
	...
	container.innerHTML = feedbacks.map(f => `
        <div class="data feedback">
            ${role === 'admin' ? `<img src="assets/trash-solid-full.svg" class="delete-data" data-id="${f.message_id}" data-table="contact_form" data-column="message_id" alt="Delete">` : ''}
            <p class="text meta">${formatDate(f.submitted_at)}</p>
            <p class="text name"><b>From:</b> ${f.username || ''}</p>
